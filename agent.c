# require "Dweetio.class.nut:1.0.0" dweet < -DweetIO(); // Basic wrapper to create an execute an HTTP GET // Will be used to set the device 's bus and stop number.
...
function on_temp(temp) {
  // Send to dweet
  // Asynchronous dweet
  dweet.dweet("travvik-temperature ", {
    "temperature ": temp
  }, function(response) {
    server.log(response.statuscode + ": " + response.body);
  });
  ...
  dweet.dweet("travvik-geolocation ", {
    "latitude ": geo_lat,
    "longitude ": geo_long
  }, function(response) {
    server.log(response.statuscode + ": " + response.body);
